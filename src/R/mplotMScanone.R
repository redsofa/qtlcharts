...
#' mplotMScanone(out, effects=eff, times=times,
         ...
         effects, chr, chartOpts=NULL)
{
    ...
    htmlwidgets::createWidget("mplotMScanone",
...
#' @export
mplotMScanone_output <- function(outputId, width="100%", height="580") {
    htmlwidgets::shinyWidgetOutput(outputId, "mplotMScanone", width, height, package="qtlcharts")
}

#' @rdname qtlcharts-shiny
#' @export
mplotMScanone_render <- function(expr, env=parent.frame(), quoted=FALSE) {
    if(!quoted) { expr <- substitute(expr) } # force quoted
    htmlwidgets::shinyRenderWidget(expr, mplotMScanone_output, env, quoted=TRUE)
}
